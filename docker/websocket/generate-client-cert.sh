...
# Used for mTLS (Mutual TLS) testing against the ws-mtls-proxy service.
#
# Prerequisites:
#   Run ./generate-cert.sh first to create certs/ca.crt and certs/ca.key.
#
# Why a proper chain?
#   The Node.js proxy uses the standard TLS API; it expects a leaf cert
#   (CA:FALSE) with extendedKeyUsage=clientAuth signed by the CA that the
#   mTLS nginx server trusts (ssl_client_certificate = ca.crt).
#
# Outputs (in ./certs):
...
openssl req -nodes -newkey rsa:2048 \
  ...
  -subj   "/CN=RedfireForge Test Client/O=RedfireForge Dev/OU=WebSocket Studio"
...
echo "Generated mTLS client credentials:"
echo "  Client cert : $CERT_DIR/client.crt  (paste into 'Client Certificate (PEM)')"
echo "  Client key  : $CERT_DIR/client.key  (paste into 'Client Private Key (PEM)')"
echo
echo "Client cert to paste into the TLS panel 'Client Certificate (PEM)' field:"
echo "  cat $CERT_DIR/client.crt"
echo
echo "Client key to paste into the TLS panel 'Client Private Key (PEM)' field:"
